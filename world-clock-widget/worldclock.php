...
class WorldClock {
  ...
  function init() {
    ...
    register_sidebar_widget(WorldClock::$registerName, array($widget,WorldClock::$functDisplay));

    // This registers our optional widget control form.
    register_widget_control(WorldClock::$registerName, array($widget,WorldClock::$functControl), WorldClock::$width, WorldClock::$height);
  }
  ...
  function displayClocks($wc_clocks, $wc_align='left', $wc_horizontal=false) {
	echo '<br/><div class="world-clock" style="text-align:'.$wc_align.';">';
    for($i=0; $i<count($wc_clocks); ++$i) {
	  if($wc_horizontal) echo '<div style="float:left;width:200px;">';
      echo '<strong class="city-name">'.$wc_clocks[$i]['city'].'</strong><div id="city'.$i.'"></div><br/>';
  ...
  function displayJavascript($options) {
    ...
    for($i=0; $i<count($wc_clocks); ++$i) {
      ...
      if($this->timezones[$wc_clocks[$i]['tz']]['daylight'] == true) {
        if($wc_clocks[$i]['dst'] == true &&
            $this->timezones[$wc_clocks[$i]['tz']]['dst_start'] < time() &&
            $this->timezones[$wc_clocks[$i]['tz']]['dst_end'] > time()) {
            $dst = 'true';
        }
      }
  ...
  function controlSaveNewClock() {
    if( !(isset($_POST['tz']) && $_POST['tz'] != -1 &&
          isset($_POST['city']) && $_POST['city'] != '') ) return;
  ...
  function controlEditExistingClock() {
    if( !(isset($_POST['worldclock_id_edit']) && $_POST['worldclock_id_edit'] != -1 &&
          isset($_POST['worldclock_tz_edit']) && $_POST['worldclock_tz_edit'] != -1 &&
          isset($_POST['worldclock_city_edit']) && $_POST['worldclock_city_edit'] != '') ) return;
  ...
  function controlRemoveExistingClock() {
    ...
    for($i=0; $i<$number_of_clocks; ++$i) {
      if( array_key_exists($i, $rm_flip) ) { echo $i; continue; }
  ...
  function controlGlobalDateTimeFormat($wc_dateformat, $wc_timeformat) {
	...
	return '<div style="float:left;margin:5px;"><div>' . $labelDate . $fmtInfo . '<br/>' . $inputDate . '</div>' .
	       '<div>' . $labelTime . $fmtInfo . '<br/>' . $inputTime . '</div></div>';
  }
  ...
  function controlGlobalTextAlign($wc_textalign) {
	...
	return '<div style="float:left;margin:5px;">' . $label . '<br/>' . $radioLeft . ' ' . $radioCenter . ' ' . $radioRight . '</div>';
  }
  ...
  function controlGlobalHorizontal($wc_horizontal) {
	...
	return '<div style="float:left;margin:5px;">' . $label . '<br/>' . $radioNo . ' ' . $radioYes . '</div>';
  }
  ...
  function controlGlobalTimeSource($wc_timesource) {
	...
	return '<div style="float:left;margin:5px;">' . $label . '<br/>' . $radioClient . ' ' . $radioServer . '</div>';
  }
...
add_action( 'widgets_init', array('WorldClock','init') ); ?>