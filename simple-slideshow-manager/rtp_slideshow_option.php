...
//***************Form data sent starts ****************
	...
	if($rtp_slideshow_imageupload_complete_data=="")
	{
		...
		if (preg_match('/[\'^£$%`!&*()}{@#~?><>,|=_+¬-]/', $rtp_slideshow_galleryname))
		{
...
?>	<form name="acurax_si_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<p class="widefat" style="padding:8px;width:99%;margin-top:8px;">	<?php _e('New Gallery Name','simple-slideshow-manager' ); ?>
			...
			<input type="submit" name="rtp_slideshow_addgallery"  value="<?php _e('Add Gallery', 'simple-slideshow-manager') ?>" id="rtp_ex_add_gallery" class="button"/>
		</p>
...
<?php			
		echo "<ul id='rtp_ex_gall' style='display:inline-block;'>";
			foreach ($rtp_slideshow_imageupload_complete_data as $key => $value)
			{
				...
				<a id="rtp_slideshow_delete_gallery" class="del_gallery button" href="?page=Acurax-Slideshow-Settings&del=<?php echo $key; ?>" alt = "Click to Delete this Gallery" onclick="return confirm('Are you sure? \n\nNOTE: You Cant Undo This Action Once Processed');"><?php _e('Delete this Gallery','simple-slideshow-manager'); ?></a>
				<a id="rtp_slideshow_manage_gallery" class="manage_gallery button"  value = "<?php echo $key?>" href="admin.php?page=Acurax-Slideshow-Add-Images&name=<?php echo $key;?>" alt = "Click to Manage this Gallery"><?php _e('Manage this Gallery','simple-slideshow-manager'); ?></a></br></br>
				<?php
				echo "</div></li>";			
			}
			echo "</ul>";
