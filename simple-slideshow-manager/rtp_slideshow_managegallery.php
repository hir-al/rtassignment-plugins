...
echo "<h2>" . __( 'Simple Slideshow Manager Settings', 'simple-slideshow-manager' ) . "</h2>"; ?>
	<form name="rtp_slideshow_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		if (isset($_GET['name'])) 
		{
			...
			?>
			<p class="widefat" style="padding:8px;width:99%;margin-top:8px; ">	<?php _e('Selected Gallery ','simple-slideshow-manager');?>: <b><?php echo $rtp_selected_gallery_name; ?></b>
			<input type = "hidden" id = "gallery_selected" name = "gallery_selected" value = "<?php  echo $rtp_selected_gallery_name;?>">			
			<span id="shortc"><b><?php _e('Shortcode:-','simple-slideshow-manager'); ?></b>
			<input type="text" value='[rtp_slideshow name="<?php  echo $rtp_selected_gallery_name;?>"]' readonly size="60" onClick="javascript:this.focus();this.select();">
		...
		<p class="widefat" style="padding:8px;width:99%;margin-top:8px; ">
			<label for="upload_image">
				<input id="rtp_slideshow_upload_image_url" readonly type="hidden" size="78" name="rtp_slideshow_upload_image" value="" />
				 <a id="rtp_slideshow_upload_image_button" class="button  rtp_slide_upload" href="#" ><?php _e('Add Image as Slide ','simple-slideshow-manager'); ?></a>
...
?>	
		...
		<div class="widefat" style="padding:8px;width:99%;margin-top:8px;" >
			<p><?php _e(' Advanced Settings','simple-slideshow-manager' ); ?> <a href="javascript:rtp_add_advance();" class="close"><b id="plus_minus">[+]</b></a></p>
			<br>
			<div id = "rtp_advance" style = "display:none" >
				<?php _e('Time span in seconds','simple-slideshow-manager'); ?> <input type = "text" name = "rtp_slideshow_timespan" id= "rtp_slideshow_timespan" size = "40" value = "<?php echo $rtp_slideshow_timespan;  ?>"/></br></br>
				<?php _e('Fadein Time in seconds','simple-slideshow-manager'); ?><input type = "text" name = "rtp_slideshow_fadeintime" id= "rtp_slideshow_fadeintime" size = "40"  value = "<?php echo $rtp_slideshow_fadeintime;?>" /></br></br>
				<?php _e('Fadeout Time in seconds','simple-slideshow-manager'); ?><input type = "text" name = "rtp_slideshow_fadeouttime" id= "rtp_slideshow_fadeouttime" size = "40"  value = "<?php echo $rtp_slideshow_fadeouttime;  ?>" /></br></br>
				...
				<option value="true" <?php if($rtp_slideshow_pauseon_hover=="true"||$rtp_slideshow_pauseon_hover==""){?> selected="selected" <?php } ?> ><?php _e('Enable','simple-slideshow-manager'); ?></option>
				<option value="false" <?php if($rtp_slideshow_pauseon_hover=="false"){?> selected="selected" <?php } ?> ><?php _e('Disable','simple-slideshow-manager'); ?></option>
				</select>
				</br></br>
				<?php 
				...
				else if(preg_match('/px/', $rtp_slideshow_height))
				{
				...
				else if(preg_match('/px/', $rtp_slideshow_width))
				{
				...
				<?php _e('Slide height','simple-slideshow-manager'); ?> <input type = "text" name = "rtp_slideshow_height" id= "rtp_slideshow_height" size = "40"  value = "<?php echo $temp_height;?>" />
					<select id="rtp_slideshow_height_type" name="rtp_slideshow_height_type">
				<option value="px" <?php if($rtp_slideshow_height_type=="px"){?> selected="selected" <?php } ?> ><?php _e('px','simple-slideshow-manager'); ?></option>
				<option value="%" <?php if($rtp_slideshow_height_type=="%"){?> selected="selected" <?php } ?>><?php _e('%','simple-slideshow-manager'); ?></option>
				</select>
				</br></br>
				<?php _e('Slide width','simple-slideshow-manager'); ?><input type = "text" name = "rtp_slideshow_width" id= "rtp_slideshow_width" size = "40"  value = "<?php echo $temp_width;?>" />
				<select id="rtp_slideshow_width_type" name="rtp_slideshow_width_type">
				<option value="px" <?php if($rtp_slideshow_width_type=="px"){?> selected="selected" <?php } ?> ><?php _e('px','simple-slideshow-manager'); ?></option>
				<option value="%" <?php if($rtp_slideshow_width_type=="%"){?> selected="selected" <?php } ?> ><?php _e('%','simple-slideshow-manager'); ?></option>
				</select>
				</br></br>
				<p class="submit">
...
<div id="rtp_upload_video"  class="widefat" style="display:none">
	...
	<input type = "hidden" id="rtp_gall_name" name="rtp_gall_name"  value = "<?php echo $_GET['name'];  ?>"/>
