...
title: '<?php
	__('Choose Image', 'simple-slideshow-manager') ?>',
button: {
text: '<?php
	_e('Choose Image', 'simple-slideshow-manager') ?>'
},
multiple: false						});
...
?>
		...
		The gallery group <b><?php
		echo $gallery_name; ?></b> is no longer available , Please reconfigure your widget/shortcode/phpcode which display's this slideshow.
		</div>
		<?php
	}
...
?>
		<div class="rtp_ss_error">
		You have not configured height or width for the slideshow group <b><?php
		echo $rtp_slideshow_imageupload_complete_data[$gallery_name]; ?></b>, Please go to wp-admin -> Slideshow -> Manage Gallery -> Advanced Settings and configure Slideshow width and height. 
		</div>
		<?php
	}
	else {
?>
	<ul class="rtp_ppt rtp_ppt_<?php
		echo $uid; ?> rtp_ppt_uid_<?php
		echo $rtp_gallery_uid; ?>" style="<?php
		if ($rtp_slideshow_height != "") { ?>height:<?php
			echo $rtp_slideshow_height; ?>; <?php
		}

		if ($rtp_slideshow_width != "") { ?> width:<?php
			echo $rtp_slideshow_width; ?>; <?php
		}
...
?>
				<a href="<?php
					echo $rtp_slideshow_link_url; ?>" target="<?php
					echo $rtp_slideshow_target; ?>" title="<?php
					echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['image_title']; ?>">
				<?php
				}

?>
			<img  src="<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['image_url']; ?>" title="<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['image_title']; ?>" alt="<?php
				$rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['image_alttext']; ?>" style="<?php
				if ($rtp_slideshow_height != "") { ?>height:<?php
					echo $rtp_slideshow_height; ?>; <?php
				}
...
?>
			...
			<div style="display:none;<?php
				...
				if ($rtp_slideshow_width != "") { ?>width:<?php
					echo $rtp_slideshow_width; ?>;<?php
				} ?>" class="rtp_v_hold" id="<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id'] . $uid; ?>">
			<iframe src="http://www.youtube.com/embed/<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>?enablejsapi=1&autoplay=1&controls=0&wmode=opaque&cc_load_policy=1&iv_load_policy=3&loop=1" width="100%" height="100%"></iframe>
			<div class="rtp_no_mo" style="height:100%;width:100%;"></div>
			</div>
			
			<img id="rtp_image_<?php
				echo $uid; ?>_<?php
				echo $i; ?>" src="<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['thumbnail_image']; ?>" <?php
				if ($rtp_slideshow_height != "") { ?>height= "<?php
					echo $rtp_slideshow_height; ?>" <?php
				}
			...
			<div class="rtp_dis_play_but rtp_dis_yplay_but_<?php
				echo $uid; ?>" title="" onclick ="call_rtp_y_player('<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>','<?php
				echo "playVideo"; ?>',<?php
				echo $i; ?>,<?php
				echo $uid; ?>);"> </div>
			<div class="player_cntrls">
			<div class="rtp_dis_pause_but rtp_dis_ypause_but_<?php
				echo $uid; ?>" title="" onclick ="call_rtp_y_player('<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>','<?php
				echo "stopVideo"; ?>',<?php
				echo $i; ?>,<?php
...
?>
			...
			<iframe src="https://player.vimeo.com/video/<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>?api=1&player_id=player&autoplay=1&title=0&byline=0&portrait=0&loop=1" <?php
				if ($rtp_slideshow_height != "") { ?>height= "<?php
					echo $rtp_slideshow_height; ?>" <?php
				}
            ...
            <div class="rtp_dis_play_but rtp_dis_vplay_but_<?php
				echo $uid; ?>" title="" onclick ="rtp_play_vimeo_video('<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>',<?php
				echo $i; ?>,<?php
				echo $uid; ?>);"> </div>
			<div class="player_cntrls">
			...
			<div class="rtp_dis_pause_but rtp_dis_vpause_but_<?php
				echo $uid; ?>" title="" onclick ="rtp_stop_vimeo_video('<?php
				echo $rtp_slideshow_imageupload_complete_data[$gallery_name][$i]['vedio_id']; ?>',<?php
				echo $i; ?>,<?php
				echo $uid; ?>);"> </div>
			</div>
...
?>
	<input type="hidden" id="rtp_hidden_id_<?php
		echo $uid; ?>" value="stop"/>
	</ul>
	<?php
...
?>
jQuery('.rtp_ppt_<?php
		echo $uid; ?> li:gt(0)').hide();
jQuery('.rtp_ppt_<?php
		echo $uid; ?> li:last').addClass('last');
var cur_<?php
		echo $uid; ?> = jQuery('.rtp_ppt_<?php
		echo $uid; ?> li:first');
var sliderHover_<?php
		echo $uid; ?> = false;
function animate_<?php
		echo $uid; ?>() 
{
if(sliderHover_<?php
		echo $uid; ?> == false)
{
cur_<?php
		echo $uid; ?>.fadeOut( <?php
		echo $rtp_slideshow_fadeouttime; ?> );
if ( cur_<?php
		echo $uid; ?>.attr('class') == 'last' )
cur_<?php
		echo $uid; ?> = jQuery('.rtp_ppt_<?php
		echo $uid; ?> li:first');
else
...
cur_<?php
		echo $uid; ?>.fadeIn(<?php
		echo $rtp_slideshow_fadeintime; ?>);
}
cur_<?php
		echo $uid; ?>.hover(
function () {
var hidden_id = 'rtp_hidden_id_'+<?php
		echo $uid; ?>;
var vedio_status = document.getElementById(hidden_id).value;
<?php
		if ($rtp_slideshow_pauseon_hover == "true") { ?>
sliderHover_<?php
			echo $uid; ?> = true;
<?php
		}
...
jQuery(function() 
{
setInterval( "animate_<?php
		echo $uid; ?>()",<?php
		echo $rtp_slideshow_timespan; ?>);
} );
<?php
	}
...

{
	...
	function rtp_Slideshow_Widget()
	{
		...
		$widget_options = array(
			'description' => __('Allow users to include a slideshow  From Acurax Slideshow Plugin', 'simple-slideshow-manager') ,
			'classname' => 'rtp_slideshow_desc'
		);
	...
	function form($instance)
	{
		...
		$defaults = array(
			'title' => __('Simple Slideshow', 'simple-slideshow-manager') ,
			'gallery_name' => '',
			'gallery_height' => '',
			'gallery_width' => '100%'
		);
...
?>
		...
		echo $this->get_field_name('title'); ?>" 
				value="<?php
		echo $instance['title']; ?>" type="text" class="widefat" />
			</p>
			<p>
				<label for="<?php
		echo $this->get_field_id('gallery_name'); ?>"><?php
		_e('Gallery Name:'); ?></label>
				<select class="widefat" name="<?php
		echo $this->get_field_name('gallery_name'); ?>" id="<?php
		echo $this->get_field_id('gallery_name'); ?>">
				<?php
		$rtp_slideshow_gallery_data = unserialize(get_option('rtp_slideshow_gallery_data'));
		$rtp_slideshow_imageupload_complete_data = unserialize(get_option('rtp_slideshow_imageupload_complete_data'));
		foreach($rtp_slideshow_imageupload_complete_data as $key => $value) {
...
?>
			...
			<p>
				<label for="<?php
		echo $this->get_field_id('gallery_height'); ?>"><?php
		_e('Slide Height:'); ?></label>
				<input id="<?php
		echo $this->get_field_id('gallery_height'); ?>" name="<?php
		echo $this->get_field_name('gallery_height'); ?>" 
				value="<?php
		echo $instance['gallery_height']; ?>" type="text" class="widefat" />
			</p>
			<p>
				<label for="<?php
		echo $this->get_field_id('gallery_width'); ?>"><?php
		_e('Slide Width:'); ?></label>
				<input id="<?php
		echo $this->get_field_id('gallery_width'); ?>" name="<?php
		echo $this->get_field_name('gallery_width'); ?>" 
				value="<?php
		echo $instance['gallery_width']; ?>" type="text" class="widefat" />
			</p>
		<?php
	}
...
function rtp_slideshow_ajax_editimage_callback()
{
	...
	<tr><td><?php
	_e('Target', 'simple-slideshow-manager'); ?></td><td><select id="rtp_link_target">
	<option value="_blank" <?php
	if ($target == "_blank") {
		echo 'selected="selected"';
	} ?>><?php
	_e('_blank', 'simple-slideshow-manager'); ?></option>
	<option value="_self" <?php
	if ($target == "_self") {
		echo 'selected="selected"';
	} ?>><?php
	_e('_self', 'simple-slideshow-manager'); ?></option>
	<option value="_parent" <?php
	if ($target == "_parent") {
		echo 'selected="selected"';
	} ?>><?php
	_e('_parent', 'simple-slideshow-manager'); ?></option>
	<option value="_top" <?php
	if ($target == "_top") {
		echo 'selected="selected"';
	} ?>><?php
	_e('_top', 'simple-slideshow-manager'); ?></option>
	</select></td></tr>
	<?php
