...
class rtp_weather_widget extends WP_Widget
{
	...
	function widget($args, $instance)
	{
		...
		if (filemtime($cache) < (time() - 20)) {
			$localZipCode = $esoid;
			$weatherXmlString = retrieveYahooWeather($localZipCode); // Itt tÃ¶lti be a fÃ¼ggvÃ©nyt
		}
		else {
		...
		while ($i < 5) {
			if ($ikon[$i] == 0) {
				$ikon[$i] = "tornado.png";
				$ikon_text[$i] = "Chance of Heavy Rain";
			}
			...
			if ($ikon[$i] == 5) {
				$ikon[$i] = "havaseso.png";
				$ikon_text[$i] = "Chance of Rain & Snow";
			}
		...
		$output = '';
		if (!empty($image)):
			if ($is_template_path == true) $image_style = "background:url(" . plugins_url(plugin_basename(dirname(__FILE__)) . '/img/' . $image) . ") no-repeat scroll 0 0 transparent";
			else $image_style = "background:url(" . $image . ") no-repeat scroll 0 0 transparent";
		else:
			$image_style = "";
		endif;
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php
		echo $this->get_field_id('esoid'); ?>"><?php
		_e('EsoID: Find the nearest forecast site to your town! <br/> Find the ID here:<a href="http://esotanc.hu/wordpress-weather-widget" TARGET="_blank">EsoID</a>', 'esoid'); ?></label></br>
			<input id="<?php
		echo $this->get_field_id('esoid'); ?>" name="<?php
		echo $this->get_field_name('esoid'); ?>" value="<?php
		echo $instance['esoid']; ?>" type="text" class="widefat" />
		</p>
		<p>
			<p>
			<label for="<?php
		echo $this->get_field_id('name'); ?>"><?php
		_e('Enter your city name:', 'name'); ?></label>
			<input id="<?php
		echo $this->get_field_id('name'); ?>" name="<?php
		echo $this->get_field_name('name'); ?>" value="<?php
		echo $instance['name']; ?>" type="text" class="widefat" />
		</p>
		</p>
		<p>
			<p>
		...
		_e('Enter image full url:', 'rtPanel'); ?></label>
			<input id="<?php
		echo $this->get_field_id('image'); ?>" name="<?php
		echo $this->get_field_name('image'); ?>" value="<?php
		echo $instance['image']; ?>" type="text" class="widefat"/>
			<input id="<?php
		echo $this->get_field_id('is_template_path'); ?>" name="<?php
		echo $this->get_field_name('is_template_path'); ?>" class="checkbox" type="checkbox" <?php
		checked($instance['is_template_path'], true) ?>/>			
			<label for="<?php
		echo $this->get_field_id('is_template_path'); ?>"><?php
		_e('Use Template Path for Image', 'rtPanel'); ?></label>
		</p>
		</p>
	<?php
	}
